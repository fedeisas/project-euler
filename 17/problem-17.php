...
?>
<?php
$strings = array( 	1=> "one", 2=> "two", 3=> "three", 4=> "four", 5=> "five",
...
function convert_num_to_string($number,$strings) {
	
	$string = "";
	
	if ($number >= 1000) {
		$thousands = intval($number/1000);
	    $string .= $strings[$thousands];
	    $string .= " thousand";
	    $number -= $thousands * 1000;
	}
    if ($number >= 100) { 
    	$hundreds = intval($number/100);
    	$string .= " ".$strings[$hundreds];
	    $string .= " hundred";
	    $number -= " ".$hundreds * 100;
    }
    if ($number) {
...
for ($i = 1 ; $i <= 1000 ; $i++ ) {
	$final_string.= convert_num_to_string($i,$strings);
}
